...
| Normal (default) | State scale is moderate, default continuous mode generates in one go; if mid-way window switch is desired, input `继续生成 .skill/ppt-master/projects/<project_name>` after Step 5 to switch to [split mode](stages/resume-execute.md). |
