...
| **B** Existing SVG assets | `./.skill/ppt-master/projects/<x>/svg_output/`, `templates/layouts/<existing>`, or a loose `.svg` folder | `ls` + `Read` each `*.svg`; plus `design_spec.md` / `spec_lock.md` if present | `standard` / `fidelity` (explicit structure first; visual clustering only when unstructured) / `mirror` (direct 1:1 copy) |
...
- **fidelity on type B** — inspect root `data-pptx-layout`, direct Master/Layout/placeholder metadata, and companion `spec_lock.md` first. When a complete explicit contract exists, its keys and `pptx_layouts` families are authoritative; preserve them instead of visual re-clustering. Strip deck-instance `data-pptx-layout-kind` when rebuilding the reusable template prototype, while retaining key/name/layers/placeholders/bounds. If the input is an existing template, use its metadata and variant filenames. Only fully unstructured SVG input falls back to AI visual clustering.
- **distilled project on type B** — when the source `svg_output/` carries `data-pptx-layout-kind`, ingest only pages explicitly selected by the user or roots marked `distilled`. Skip every `utility` page; it is an empty mapping shell, not a reusable template design. Deduplicate repeated members by explicit Layout key/contract before rebuilding prototypes, then strip the deck-instance kind. A final distilled composite `<g data-pptx-placeholder="object">` is a deck-instance region proxy and cannot survive that stripping: keep the group as the complete visual sample, remove its placeholder attributes, and rebuild its bounds as a separate direct atomic no-fill/no-line object-slot proxy in the reusable template SVG.
- **selected free-design subset on type B** — if a cross-project request names pages from an undistilled free-design project, return to [`distill-layouts`](./distill-layouts.md) first and distill exactly that source subset. Type B intake then receives only the selected `distilled` files. Do not scan or copy the whole `svg_output/` directory and silently turn unselected pages into template variants.
...
**Native structure reconstruction**: Do not copy `native_structure.json` or `source_template.pptx` into the final template directory. Use their master/layout/placeholder facts to reconstruct one clean Master plus semantic Layouts through the explicit SVG metadata contract. Materialize at least one complete template page for every retained source Layout, including unused Layouts. Immediately below `design_spec.md §V Page Roster`, write a `Source Layout Consolidation` table with one row per retained source Layout: source key/name, output SVG, rebuilt key/picker name, or an explicit unused reason. Never silently drop or merge a source Layout. Mirror keeps its exact source-slide count, so report any unused source Layout that cannot be materialized and use fidelity instead when the full Layout picker roster is required. Every emitted SVG repeats the identical Master layer, the selected Layout layer, and semantic Slide placeholders so it remains a complete standalone preview.

Downstream, both template-adherence choices use `pptx_structure.mode: template` plus `layout_strategy: distill`. `page_layouts` selects one complete input prototype per page; `pptx_layouts` is omitted until the generated SVG pages are complete. Strict restores the prototype contract. Adaptive keeps the prototype Master structure and may distill a new Layout under a new key/name. Neither falls back to baseline inside a template deck. For mirror packages, reused Master/Layout identities retain literal visual styling in addition to structure; only visible text content may change.
...
3. Placeholder vocabulary — pages should adopt the conventional names (`{{TITLE}}`, `{{CONTENT_AREA}}`, ...) when they fit. Full reference: [Placeholder Reference](../references/template-designer.md#4-placeholder-reference-canonical-convention-overridable-per-template). When a template style legitimately needs different vocabulary (consulting → `{{KEY_MESSAGE}}`, branded cover → `{{BRAND_LOGO}}`), declare a `placeholders:` block in `design_spec.md` frontmatter so the registrar and quality checker treat it as the template's authoritative contract. **Avoid** one-off indexed families such as `{{CHAPTER_01_TITLE}}` — use the indexed TOC pattern instead.
   - `{{...}}` placeholders are the authoring vocabulary used to generate final slide content. Each emitted SVG also carries the native reconstruction contract: root `data-pptx-layout` / `data-pptx-layout-name`, direct Master/Layout layers, and direct semantic `data-pptx-placeholder` markers with explicit design-zone bounds. Minimal structural `data-pptx-role` hints are added only when those specialized markers cannot express required behavior. Both strict and adaptive downstream set `mode: template`, require complete `page_layouts`, and defer `pptx_layouts` until post-design distillation.
...
- emit `placeholder_bounds_missing` for legacy packages; treat that warning as blocking in this creation workflow and add finite positive design-zone bounds to every new placeholder
...
- [ ] Every placeholder declares explicit design-zone bounds; no bound was derived from sample text length, glyph ink, or current wrapping
- [ ] Every retained source Layout is represented by a complete template page, or `design_spec.md` records its explicit consolidation target/unused reason in `Source Layout Consolidation`
