...
Three artifacts under `./.skill/ppt-master/projects/`:

| Artifact | Path |
|---|---|
| Research document | `./.skill/ppt-master/projects/<topic_slug>.md` |
| Fact provenance | `./.skill/ppt-master/projects/<topic_slug>.facts.json` |
| Image folder | `./.skill/ppt-master/projects/<topic_slug>/` |

**Hard rule — naming**: filename (without `.md`) and folder name MUST match. **Hard rule — location**: under `./.skill/ppt-master/projects/`, never the repository root.
...
```bash
mkdir -p "./.skill/ppt-master/projects/<topic_slug>"
curl -L -o "./.skill/ppt-master/projects/<topic_slug>/<descriptive_name>.<ext>" "<image_url>"
```
...
- [x] Document: `./.skill/ppt-master/projects/<topic_slug>.md` (N sections)
- [x] Facts: `./.skill/ppt-master/projects/<topic_slug>.facts.json` (N external facts)
- [x] Images: `./.skill/ppt-master/projects/<topic_slug>/` (N files)
- [ ] **Next**: [`generate-pptx`](../generate-pptx.md) Step 2 →
  `project_manager.py init <project_name> --format <format>`
  `project_manager.py import-sources ./.skill/ppt-master/projects/<project_name> ./.skill/ppt-master/projects/<topic_slug>.md ./.skill/ppt-master/projects/<topic_slug>.facts.json ./.skill/ppt-master/projects/<topic_slug>/*.* --move`
```

`<project_name>` is the user's chosen project identifier (typically `<format>_<topic_slug>`, e.g. `ppt169_joe_hisaishi`); `--move` removes the research artifacts from `./.skill/ppt-master/projects/<topic_slug>` after they are imported and deletes the folder itself once it is empty.
